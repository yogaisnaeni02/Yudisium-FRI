...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex items-center justify-between">
        ...
        <a href="{{ route('admin.periodes.create') }}" class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2.5 px-6 rounded-lg transition shadow-sm hover:shadow flex items-center gap-2">
                    ...
                    @forelse($periodes as $periode)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2 flex">
                                <a href="{{ route('admin.periodes.edit', $periode) }}" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg transition shadow-sm hover:shadow">
                                ...
                                <form action="{{ route('admin.periodes.delete', $periode) }}" method="POST" class="inline" onsubmit="return confirm('Apakah anda yakin ingin menghapus periode ini?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg transition shadow-sm hover:shadow">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
