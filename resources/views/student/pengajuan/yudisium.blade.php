...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-green-600 to-green-700 rounded-xl shadow-lg p-6 text-white">
        <h2 class="text-2xl font-bold mb-2">Pengajuan Yudisium</h2>
        <p class="text-green-100">Unggah semua berkas yang diperlukan untuk pengajuan yudisium Anda</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <aside class="lg:col-span-1">
            <div class="bg-white rounded-xl shadow-sm border border-green-200 sticky top-8">
                <!-- Sidebar Header -->
                <div class="bg-gradient-to-r from-green-600 to-green-700 text-white p-5 rounded-t-xl">
                    ...
                    <p class="text-sm text-green-100 mt-1 font-medium">Progress: {{ $progress }}%</p>
                </div>
                ...
                <div class="p-5 space-y-6">
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-3">
                            <span class="text-xs font-semibold text-gray-700">Kelengkapan Dokumen</span>
                            <span class="text-sm font-bold text-green-600">{{ $progress }}%</span>
                        </div>
                        <div class="w-full bg-green-100 rounded-full h-2.5">
                            <div class="bg-gradient-to-r from-green-500 to-green-600 h-2.5 rounded-full transition-all duration-500" style="width: {{ $progress }}%"></div>
                        </div>
                    </div>

                    <!-- Document Groups Navigation -->
                    <div>
                        <h4 class="text-xs font-bold text-gray-700 uppercase tracking-wide mb-3">Kategori Berkas</h4>
                        <nav class="space-y-2">
                            @foreach($groups as $groupName => $docs)
                                @php $groupSlug = \Illuminate\Support\Str::slug($groupName); @endphp
                                <button type="button" onclick="showSection('{{ $groupSlug }}')" data-section="{{ $groupSlug }}" class="w-full text-left p-3 rounded-lg hover:bg-green-50 transition border-l-4 border-green-300 section-btn hover:border-green-500">
                                    <div class="flex items-center justify-between">
                                        <span class="text-sm font-semibold text-gray-900">{{ $groupName }}</span>
                                        <span class="text-xs font-bold text-green-700 bg-green-100 px-2 py-1 rounded-full">{{ count($docs) }}</span>
                                    </div>
                                </button>
                            @endforeach
                        </nav>
                    ...
                    <div class="pt-4 border-t border-gray-200">
                        <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                            <p class="text-xs font-bold text-green-900 mb-3 flex items-center gap-2">
                            ...
                            <ul class="text-xs text-green-800 space-y-2">
                                <li class="flex gap-2">✓ Semua berkas wajib dilengkapi</li>
                                <li class="flex gap-2">✓ Format: PDF, DOC, DOCX</li>
                                <li class="flex gap-2">✓ Maksimal 5MB per file</li>
                            </ul>
                        </div>
                    </div>
                </div>
        ...
        <div class="lg:col-span-4 space-y-4">
                    @foreach($groups as $groupName => $docs)
                        @php $groupSlug = \Illuminate\Support\Str::slug($groupName); @endphp
                        <section class="section-content bg-white rounded-lg shadow mb-6 overflow-hidden transition-all duration-300 border border-green-200 {{ $loop->first ? '' : 'hidden' }}" data-section="{{ $groupSlug }}" id="group-{{ $groupSlug }}">
                            <div class="bg-gradient-to-r from-green-50 to-white p-6 border-b border-green-200">
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        <h4 class="text-xl font-bold text-green-900">{{ $groupName }}</h4>
                            ...
                            <div class="p-6 space-y-6">
                                <form onsubmit="uploadSection(event, '{{ $groupSlug }}')" enctype="multipart/form-data" class="space-y-6" data-group-form>
                                    ...
                                    <div class="flex items-center gap-3">
                                        <x-button variant="success" type="submit" size="lg" icon='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>'>
                                            Unggah Semua Berkas Seksi Ini
                                        </x-button>
                                        <x-button variant="ghost" size="sm" type="button" onclick="document.querySelector('#group-{{ $groupSlug }} [data-group-form]').reset()">
                                            Reset Form
                                        </x-button>
                                    </div>
                                </form>
...
<script>
    ...
    function showSection(slug) {
        ...
        document.querySelectorAll('.section-btn').forEach(b => b.classList.remove('ring-2', 'ring-green-400'));
